...
skills/
  data_acquisition/        数据获取 — 按workflow层级组织
    01_socioeconomic/      WorldPop（人口网格数据）
    04_impact/             GBD Results（健康数据）
  model/                   模型分析 — 按模型类型组织
    IAM/                   综合评估模型
      gcam/                GCAM 情景分析、配置、数据提取
...
| Skill | 所属层 | 功能 |
|-------|--------|------|
| WorldPop | 01 社会经济 | 按国家、年份下载人口网格数据，支持裁切校验 |
| GBD Results | 04 影响评估 | 从 IHME GBD 获取死亡率、疾病负担参数 |
| GCAM | IAM | 探索 BaseX 输出结构，解读配置，通过 XQuery 提取价格/产量 |
